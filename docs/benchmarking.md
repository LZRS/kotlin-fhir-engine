...
asserts its two arms produce *different* query plans, because an index created over a column
inherits that column's collation whatever the arm intended, and two identical arms compare
cheerfully to zero difference.
...
difference between a scan and a seek. **39.8x at 50,000 rows.** The engine ships the `binary` arm;
see [The two remaining shortfalls](#the-two-remaining-shortfalls).
...
SQLite applies its LIKE optimisation only when the pattern is a literal or a plain parameter, and it
uses an index only when the index collation matches the comparison's.
`StringIndexCollationBenchmark` measures the pair at 39.8x on a 50,000-row table. Adopting it would
cost `:exact` its index — the two cannot both be indexed without a second column mirroring
`index_value` — and a collation change needs a schema version bump.
...
not tell a working index from a missing one. The prefix-search change above measured as *no change
at all* there and was nearly discarded on the strength of it. Widening the corpus to 676 distinct names
...
six combinations aborted in setup. Gradle reported `BUILD SUCCESSFUL`.
