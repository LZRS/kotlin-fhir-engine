...
class TimestampBasedDownloadWorkManagerImpl(private val dataStore: DemoDataStore) :
  ...
  override suspend fun processResponse(response: Resource): Collection<Resource> {
    ...
    if (response is OperationOutcome) {
      throw IllegalArgumentException(response.issue.first().diagnostics?.value)
    }
    ...
    if (response is com.google.fhir.model.r4.List) {
      for (entry in response.entry) {
        val reference = Reference(reference = entry.item.reference)
        if (reference.reference?.value?.substringBefore("/") == "Patient") {
    ...
    if (response is Bundle) {
      val nextUrl = response.link.firstOrNull { component -> component.relation.value == "next" }?.url
      if (nextUrl != null) {
        urls.add(nextUrl.value!!)
      }
    }
  ...
  private suspend fun extractAndSaveLastUpdateTimestampToFetchFutureUpdates(
    ...
    resources
      .groupBy { getResourceType(it::class) }
      .entries
      .map { map ->
        dataStore.saveLastUpdatedTimestamp(
          map.key,
          map.value.maxOfOrNull { (it.meta?.lastUpdated?.value as FhirDateTime.DateTime).dateTime }?.toTimeZoneString() ?: "",
        )
      }
  }
}
...
private fun LocalDateTime.toTimeZoneString(): String =
