...
interface FhirEngine {
  ...
  /**
   * Returns the most recent `lastUpdated` timestamp among locally stored resources of [type], or
   * `null` if none have been synced yet. Derived directly from the resources themselves, so the
   * value can never drift from what's actually persisted locally.
   */
  suspend fun getLastUpdated(type: ResourceType): OffsetDateTime?
